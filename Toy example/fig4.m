...
b=12;
phat = a/(a+b) % mean 
varp = a*b/((a+b)^2*(a+b+1)) % 0.0026
pgrid = linspace(0,1,91);
fpplot = betapdf(pgrid,a,b);

% V1 distribtuion - normal (80, 40)
mu_v = 80;
sigma_v = 40;
vgrid = linspace(-50,400,91);
...
%% Fig 4 (b)- 2d informantion density
...
EUI = integral2(EUIdpv1,0,1,-50,200)
...
ax = [0.80, pmax];
ay = [80,vmax];
anArrow = annotation('arrow','Color','b','LineWidth',2);
anArrow.Parent = gca;
anArrow.X = ax; % set the x-property
anArrow.Y = ay;

text(pmax+0.01,vmax,['(',num2str(pmax,2),',',num2str(vmax,3),')'],'Interpreter','latex',FontSize=15,Color='b')
text(0.5,80,['(',num2str(0.8,2),',',num2str(80),')'],'Interpreter','latex',FontSize=15,Color='b')
